...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  var unString=palabras[0];
  for(let i=1; i<palabras.length; i++){
    unString=(unString+" "+palabras[i]);
  }
  return unString;
}
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  var unString=n.toString();
  if(unString[0]==="9") return true;
  return false;
}
...
function mesesDelAño(array) {
  ...
  if (!estaEnero||!estaMarzo||!estaNoviembre) return "No se encontraron los meses pedidos";
  return meses;
}
...
function mayorACien(array) {
  ...
  for(let i=0; i<array.length; i++ ){
    if(array[i]>100) numerosMayorACien.push(array[i]);
  }
